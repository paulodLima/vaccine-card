...
class CompositionControllerTest {
    ...
    @Test
    void getCompositions() throws Exception {

        mockMvc.perform(get("/v1/api/composition")
                .accept(MediaType.APPLICATION_JSON)
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].description", is("Virus vivo atenuado")))
                .andExpect(jsonPath("$[0].uuid", is("45b7d9a5-c2f1-4332-9e4e-d11733f67ce0")))
                .andExpect(jsonPath("$[1].description", is("Bacteria viva atenuada")))
