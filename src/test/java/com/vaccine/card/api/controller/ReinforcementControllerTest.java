...
class ReinforcementControllerTest {
    ...
    @Test
    void getReinforcements() throws Exception {

        mockMvc.perform(get("/v1/api/reinforcement")
                .accept(MediaType.APPLICATION_JSON)
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].description", is("4 anos de idade")))
                .andExpect(jsonPath("$[0].uuid", is("45b7d9a5-c2f1-4332-9e4e-d11733f67ce0")))
                .andExpect(jsonPath("$[1].description", is("12 meses")))
